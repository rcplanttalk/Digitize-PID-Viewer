...
def validate_pid_logic(G: nx.DiGraph) -> list[str]:
    ...
    for node, data in G.nodes(data=True):
        ...
        if ntype == "equipment" and cid in (24, 25):
            if G.in_degree(node) != 1 or G.out_degree(node) != 1:
                issues.append(
                    f"E002: Pump '{node}' degree constraint violated "
        ...
        if node_size is not None:
            for nbr in list(G.predecessors(node)) + list(G.successors(node)):
                ...
                if (
                    edge_type == "process"
                    and edge_size is not None
                    and edge_size != node_size
                ):
    ...
    for u, v, edata in G.edges(data=True):
        ...
        if edata.get("type") == "process" and edge_spec is not None:
            if edge_spec not in PIPE_SPEC_CODES:
                issues.append(
    ...
    non_off_page = [
        n for n, d in G.nodes(data=True) if d.get("type") != "off_page"
    ]
    ...
    loop_nodes = [
        ...
        if d.get("class_id") == 3 or d.get("type") == "control_valve"
    ]
    ...
    for node, data in G.nodes(data=True):
        if data.get("class_id") == 8:
            ...
            has_flare = any(
                G.nodes[s].get("type") in ("off_page", "flare", "vent")
                for s in successors
            )
            if not has_flare:
                issues.append(
